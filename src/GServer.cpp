...
// 
// GServer::GServer(std::shared_ptr<spdlog::logger> log, std::vector<std::string> maps)
// 	: Server(log)
// {
...
// // 				log->info(fmt::format("sqldb: %s", sqldb));
// // 
...
// // 						catch (...)
// // 						{
// // 							log->critical(fmt::format("Failed to load map: %s", mapname));
// // 							return false;
// // 						}
...
// // 						catch (...)
// // 						{
// // 							log->info(fmt::format("e: item: %s", newitem->name));
// // 						}
// // 
...
// // 						catch (...)
// // 						{
// // 							log->info(fmt::format("e: magic: %s", newmagic->name));
// // 						}
// // 
...
// // 
// // 		{
// // 			log->info("Loading Drop Rate JSON.");
...
// // 							}
// // 							if (npc != 0) {
// // 								//log->info(fmt::format("Processing items for NPC: %?d", npc->Type()));
...
// // 										}
// // 										if (item != 0) {
// // 											//log->info(fmt::format("Setting drop data; NPC: %?d, Item: %?d, Probability: %?d", npc->Type(), item->m_sIDnum, itemProb));
...
// 		}
// 	}
// 	log->info(fmt::format("Could not find item %s", name));
// 	return 0;
// }
// 
...
// // 					{
// // 						string logmsg = fmt::format("(!) Client(%s) Timeout/Hack(1400+) %?d Time gap %?d", msg->client->m_cCharName, 
// // 							msg->client->m_timeHackDif/4,   timegapclient - timegapserver);
// // 						poco_error(*log, logmsg);
// // 						poco_error(*filelog, logmsg);
// // 
// // 						if(msg->client->m_timeHackDif > 2500*4)
// // 						{
// // 							string logmsg = fmt::format("(!) Client(%s) Speed hacker timeout(3000+) %?d Time gap %?d. Banned to BI", msg->client->m_cCharName, 
// // 								msg->client->m_timeHackDif/4,   timegapclient - timegapserver);
// // 							poco_error(*log, logmsg);
// // 							poco_error(*filelog, logmsg);
...
// 					int8_t msgtype = 0;
// 
// 					chathandler->ParseChat(msg->client, message);
// 				}
// 				catch (int test)
// 				{
// 					//-193 data length error
// 					log->error(fmt::format("ChatThread() : %?d", test));
// 				}
// 			}
// 		}
// #ifdef WIN32
...
// 		catch (...)
// 		{
// 			log->critical("Unspecified GServer::TimerThread() Exception.");
// 			return;
// 		}
// 	}
...
// 	sY = client->y;
// 
// 	bRet = client->map->bSearchTeleportDest(sX, sY, cMapName, &destx, &desty, &direction);
// 
...
// 				PlayerMapEntry(client->shared_from_this(), setRecallTime);
...
// }
// 
// void GServer::PlayerMapEntry(std::shared_ptr<Client> client, bool setRecallTime)
// {
// 	if (!client) return;
// 
// 	iSetSide(client.get());
...
// 
// 	sw.WriteInt(uint32_t(client->handle));
...
// 
// 		poco_warning(*log, fmt::format("Char(%s)-Enter(%s) Observer(%?d)", (string)player->m_cCharName, (string)player->m_cMapName, player->m_bIsObserverMode));
// 	}
// 
// 	if (player->GetParty())
// 	{
...
// 	
// 	if ((client->level > DEF_LIMITHUNTERLEVEL) && (client->civilian == false) ) return;
// 
// 
// 	if (client->faction == "aresden")
// 	{
// 		client->faction = "arehunter";
// 		client->civilian = true;
// 	}
...
// 	std::shared_ptr<Client> lock;
// 
// 	try
// 	{
// 		lock = to->shared_from_this();
// 	}
// 	catch (boost::exception& e)
// 	{
// 		log->error("SendNotifyMsg() - to->shared_from_this()");
// 		return;
// 	}
// 	catch (...)
// 	{
// 		log->error("SendNotifyMsg() - to->shared_from_this() generic");
// 		return;
// 	}
// 
...
// }
// 
// void GServer::InitPlayerData(std::shared_ptr<Client> client)
// {
// 	try
// 	{
// 		{
...
// 			sw.WriteInt(uint32_t(client->stamina));
// 
...
// 
// 			sw.WriteShort(client->levelUpPool);
// 			sw.WriteInt64(client->experience);
// 			sw.WriteInt(client->enemyKillCount);
// 			sw.WriteInt(client->playerKillCount);
// 			sw.WriteInt(client->m_iRewardGold);
// 
// 			sw.WriteString(client->faction, 10);
// 			if (client->guild)
// 				sw.WriteString(client->guild->name, 20);
// 			else
// 				sw.WriteString("NONE", 20);
...
// 			sw.WriteShort(MSGTYPE_CONFIRM);
// 
// 			//TODO: will this need to be 64bit? do some mass spawn testing to see if numbers even break 32bit boundaries
// 			sw.WriteInt(uint32_t(client->charid));
// 
...
// 
// 
// 			if (client->map->fixedDay) //day(1) or night(0)?
// 				sw.WriteByte(1);
// 			else
// 				sw.WriteByte(m_cDayOrNight);
// 
...
// 	catch (std::exception& e)
// 	{
// 		log->error(fmt::format("InitPlayerData() Exception: %s", (string)e.what()));
// 	}
// }
// 
// void GServer::ClientMotionHandler(std::shared_ptr<Client> client, std::shared_ptr<MsgQueueEntry> msg)
// {
...
// }
// 
// int GServer::iClientMotion_Stop_Handler(std::shared_ptr<Client> client, uint16_t sX, uint16_t sY, uint8_t cDir)
// {
...
// }
// 
// int GServer::iClientMotion_GetItem_Handler(std::shared_ptr<Client> client, uint16_t sX, uint16_t sY, uint8_t cDir)
// {
// 	uint32_t * dwp;
// 	uint16_t  * wp;
...
// }
// 
// int GServer::iClientMotion_Attack_Handler(std::shared_ptr<Client> client, uint16_t sX, uint16_t sY, uint16_t dX, uint16_t dY, int16_t wType, uint8_t cDir, uint64_t wTargetObjectID, bool bResponse, bool bIsDash)
// {
// 	char cData[100];
// 	uint64_t dwTime;
...
// 				log->info(fmt::format("(!) Speed hack suspect(%s) - attack-lat(%?d)", client->name, dwTime - client->m_dwAttackLAT));
// 				DeleteClient(client->shared_from_this(), true, true);
// 				return 0;
// 			}
// 		}
...
// }
// 
// int GServer::iClientMotion_Move_Handler(std::shared_ptr<Client> client, uint16_t sX, uint16_t sY, uint8_t cDir, bool bIsRun)
// {
// 	uint64_t dwTime;
// 	short dX, dY, sDOtype;
// 	int   iDamage;
...
// 					log->info(fmt::format("(!) Speed hack suspect(%s) - move-lat(%?d)",
...
// 		client->SWrite(sw);
// 
// 		std::shared_ptr<Unit> owner = client->map->GetOwner(dX, dY);
// 		if(owner)
// 		{
// 			if(owner->IsPlayer())
...
// }
// 
// int GServer::iClientMotion_Magic_Handler(std::shared_ptr<Client> client, uint16_t sX, uint16_t sY, uint8_t cDir)
// {
// 	if ((!client) || (client->IsDead()) || (!client->m_bIsInitComplete)) return 0;
// 
// 	if ((sX != client->x) || (sY != client->y)) return 2;
// 
// 	if (!client->map) return 0;
// 
// 	client->map->IncPlayerActivity(client.get());
// 
// 	if (client->skillInUse[19])//TODO: which skill was this again?
// 	{
// 		client->map->ClearOwner(sX, sY);
// 		client->map->SetOwner(client, sX, sY);
// 	}
...
// }
// 
// void GServer::ClientCommonHandler(std::shared_ptr<Client> client, std::shared_ptr<MsgQueueEntry> msg)
// {
// 	if (!msg->client->m_bIsInitComplete) return;
// 	if (msg->client->_dead) return;
// 
...
// 
// 		//int32_t iV4 = sr.ReadInt();
// 		//if (client->m_hasPrecasted || client->IsGM())
// 			PlayerMagicHandler(client, iV1, iV2, (iV3 - 100));
// 	}
// 		break;
// // 
...
// // 		break;
// 	default:
// 		log->error(fmt::format("Unknown common packet received from client - 0x%.2X", command));
// 		break;
// 	}
// }
// 
...
// void GServer::PlayerMagicHandler(std::shared_ptr<Client> client, uint16_t dX, uint16_t dY, int16_t sType, bool bItemEffect /*= false*/, int32_t iV1 /*= 0*/)
// {
...
// 	{
// 		log->error(fmt::format("Magic casted out of range - Player: %s : (%d,%d) -> (%d,%d)", player->name, player->x, player->y, dX, dY));
// 		return;
// 	}
// 
...
// 				}
// 
// 				if (std::shared_ptr<Npc> npc = owner->map->CreateNpc(cNpcName, 0,
...
// }
// 
// void GServer::ToggleCombatModeHandler(std::shared_ptr<Client> client)
// {
// 	short sAppr2;
// 
// 	if (client == nullptr) return;
...
// }
// 
// bool GServer::_bAddClientItemList(std::shared_ptr<Client> client, Item * pItem, int * pDelReq)
// {
// 	int i;
// 
...
// }
// 
// int GServer::_iCalcMaxLoad(std::shared_ptr<Client> client)
// {
// 	return (client->GetStr() * 500 + client->level * 2000); // Changed max weight from 5 to 20 per level xRisenx
// }
// 
// int GServer::iCalcTotalWeight(std::shared_ptr<Client> client)
// {
// 	client->UpdateWeight();
// 
...
// }
// 
// void GServer::DropItemHandler(std::shared_ptr<Client> client, short sItemIndex, int iAmount, string pItemName, bool bByPlayer)
// {
// 	Item * pItem;
...
// }
// 
// void GServer::SendItemNotifyMsg(std::shared_ptr<Client> client, uint16_t msgtype, Item *pItem, int iV1)
// {
// 	char  * cp, cData[512];
// 	uint32_t * dwp;
// 	uint16_t  * wp;
...
// }
// 
// int GServer::SetItemCount(std::shared_ptr<Client> client, int32_t iItemID, uint32_t dwCount)
// {
// 	uint16_t wWeight;
// 
// 
...
// }
// 
// void GServer::ItemDepleteHandler(std::shared_ptr<Client> client, Item * pItem, bool bIsUseItemResult, bool bIsLog, bool notify)
// {
// 	if (client->m_bIsInitComplete == false) return;
// 
// /*
...
// }
// 
// void GServer::RequestFullObjectData(std::shared_ptr<Client> client, Unit * target)
// {
// 	uint32_t temp;
// 
// 	if (client == 0) return;
...
// }
// 
// bool GServer::LoadCharacterData(std::shared_ptr<Client> client)
// {
// 	try
// 	{
// 		{
...
// 			client->stamina = rs.value("sp").convert<uint64_t>();
...
// void GServer::DeleteClient(std::shared_ptr<Client> client, bool save, bool deleteobj)
// {
// 	string cTmpMap;
...
// 	if (!client) return;
// 
// 	lock_guard<shared_mutex> lock(Gate::GetSingleton().mutclientlist);
// 
// 	client->socket->stop();
// 
...
// 
// 		for (std::shared_ptr<Client> target : clientlist)
// 		{
...
// 			lock_guard<mutex> lock(mutchat);
// 			for (std::list<std::shared_ptr<MsgQueueEntry>>::iterator iter = chatpipe.begin(); iter != chatpipe.end();)
// 			{
...
// // 		if (dwTimeGap < 320) {
// // 			log->info(fmt::format("(!) Speed hack suspect(%s) - attack(%?d)", client->name, dwTimeGap));
// // 			return false;
// // 		}
// // 		else if (dwTimeGap < 240) {
// // 			log->info(fmt::format("(!) Speed hack suspect(%s) - attack(%?d). Disconnected", client->name, dwTimeGap));
// // 			DeleteClient(client->shared_from_this(), true, true);
// // 			return false;
// // 		}
// // #endif
...
// 		if (dwTimeGap < 1200) {
// 			log->info(fmt::format("(-~-HACKING-~-) Speed hacker detected(%s) - magic(%?d). Disconnected", client->name, dwTimeGap));
// 			DeleteClient(client->shared_from_this(), true, true);
// 			return false;
// 		}
// 		else if (dwTimeGap < 1600) {
// 			log->info(fmt::format("(-~-HACKING-~-) Speed hack suspect(%s) - magic(%?d)", client->name, dwTimeGap));
// 			return false;
// 		}
// #endif
// 	}
...
// 				{
// 					log->info(fmt::format("(-~-HACKING-~-) Speed hacker detected(%s) - run-avg(%?d). BI banned", client->name, sum / SPEEDCHECKTURNS));
// 
// 					DeleteClient(client->shared_from_this(), true, true);
// 					return false;
// 				}
...
// 				{
// 					log->info(fmt::format("(-~-HACKING-~-) Speed hack suspect(%s) - run-avg(%?d)", client->name, sum / SPEEDCHECKTURNS));
// 
...
// 				{
// 					log->info(fmt::format("(-~-HACKING-~-) Speed hack suspect(%s) - move-avg(%?d)", client->name, sum / SPEEDCHECKTURNS));
// 
...
// 				}
// 			}
// 			if (player->map->iGetAttribute(target->x, target->y, 0x00000005) != 0) return;
// 		}
// 
...
// 		}*/
// 
// 		damage -= dice(1, player->GetVit() / 10) - 1;
// 		if (damage < 0) damage = 0;
// 
...
// 				SendEventToNearClient_TypeA(player, MSGID_MOTION_DAMAGE, damage, 0, 0);
// 			}
// 		}
// 	}
// 	else
...
// 
// 						ItemDepleteHandler(cattacker->shared_from_this(), cattacker->Equipped.Arrow, false);
// 
...
// 						iCalcTotalWeight(cattacker->shared_from_this());
// 					}
// 				}
...
// 				ntarget->health = 0;
// 				if (attacker->IsPlayer())
// 					target->map->NpcKilledHandler(static_cast<Client*>(attacker)->shared_from_this(), static_cast<Npc*>(target)->shared_from_this(), iDamage);
// 				else
// 					target->map->NpcKilledHandler(static_cast<Npc*>(attacker)->shared_from_this(), static_cast<Npc*>(target)->shared_from_this(), iDamage);
// 
// 				bKilled = true;
...
// 
// 	std::shared_ptr<Unit> target = client->map->GetOwner(dX, dY);
// 
...
// 			tY = (int)client->y;
// 
// 			std::shared_ptr<Npc> guard = client->map->CreateNpc(cNpcName, 0, MOVETYPE_RANDOM,
// 									 &tX, &tY, (Side)-1, cNpcWaypoint, 0, 0, false, true);
// 
// 			if (guard)
// 			{
// 				guard->SetTarget(client->shared_from_this());
// 			}
// 		}
// 	}
// 
