...
npc::npc(uint64_t NpcH, map * map_)
    : unit(NpcH)
    , map_(map_)
{
    set_owner_type(ot_npc);

    for (int i = 0; i < MAXWAYPOINTS; i++)
        m_iWayPointIndex[i] = -1;

    for (int i = 0; i < MAXMAGICEFFECTS; i++)
...
bool npc::initNpcAttr(npc * npcTemplate, char cSA)
{
    ...
    if (npcTemplate != 0)
    {
        ...
        set_type(npcTemplate->get_type());

        experience = utility::gen_rand(npcTemplate->m_iExpDice, 4) + npcTemplate->m_iExpDice;
        ...
        m_iAttackStrategy = utility::gen_rand(1, 10);
        m_iAILevel = utility::gen_rand(1, 3);
        ...
        switch (specialAbility)
        {
            ...
            case 3: // Absorbing Physical Damage
                if (absDamage > 0) {
                    ...
                    iTemp = 20 + utility::gen_rand(1, 60);
                    absDamage -= iTemp;
            ...
            case 9://?? is this 'highly trained'?
                ...
                m_cResistMagic *= 1.5 + double(mult / 10);
        ...
        if (m_iHitDice <= 5)
            health = abs(utility::gen_rand(m_iHitDice, 4) + m_iHitDice);
        else health = abs(int64_t(m_iHitDice) * 5 + utility::gen_rand(1, m_iHitDice));

        if (health == 0) health = 1;

        m_iNoDieRemainExp = (experience)-(experience / 3);

        utility::SetNibble(status, 2, cSA);

        if (side == 10) // temporary code
            side = faction::hostile;
        utility::SetNibble(status, 7, side);
...
void npc::RegenHP()
{
    ...
    uint64_t now = utility::current_time();
...
void npc::behavior_move()
{
    ...
    if (magic_effect_status[MAGICTYPE_HOLDOBJECT] != 0) return;
    if ((target != nullptr) && (target->map_->name != map_->name))
    {
        map_->RemoveFromTarget(this->shared_from_this());
        return;
    }
    ...
    if (sTarget != nullptr)
    {
        if (timeActionInterval < 1000)
        {//Changed to 500 for quicker re-action time.. 1000 seems too slow
            if (utility::gen_rand(1, 3) == 3)
            {
                ...
                targetType = sTarget->get_owner_type();

                return;
            }
        }
        else
        {//I don't think this was even needed? Either way it's kind of stupid whoever did this.
            m_cBehavior = BEHAVIOR_ATTACK;
            m_sBehaviorTurnCount = 0;
            target = sTarget;
            targetType = sTarget->get_owner_type();
    ...
    if ((isLeader == true) && (utility::gen_rand(1, 3) == 2)) return;
    ...
    if (m_cMoveType == MOVETYPE_FOLLOW)
    {
        ...
        {
            short DOType = 0;
            cDir = GetNextMoveDir(sX, sY, dX, dY, map_, turn, &m_tmp_iError, &DOType);
            ...
            else
            {
                if (DOType == DYNAMICOBJECT_SPIKE)
                {
                    ...
                    if (is_dead())
                    {
                        map_->NpcKilledHandler(nullptr, sft(), 0);
                        return;
                    }
                    else
                    {
                        map_->gs->send_event_near_type_a(shared_from_this(), MSGID_MOTION_DAMAGE, dmg, 1, 0);
                    }
                }
                ...
                map_->ClearOwner(x, y);
                map_->SetOwner(shared_from_this(), dX, dY);

                x = dX;
                y = dY;
                direction = cDir;
                map_->gs->send_event_near_type_a(shared_from_this(), MSGID_MOTION_MOVE, 0, 0, 0);
            }
        }
    }
    else
    {
        short DOType = 0;
        cDir = GetNextMoveDir(x, y, m_dX, m_dY, map_, turn, &m_tmp_iError, &DOType);

        if (cDir == 0)
        {
            if (utility::gen_rand(1, 10) == 3) nextWaypointDest();
        }
        else
        {
            if (DOType == DYNAMICOBJECT_SPIKE)
...
void npc::behavior_flee()
{
    ...
    if (this->m_sBehaviorTurnCount > 10)
    {
        ...
        if (health <= 3)
        {
            health += utility::gen_rand(1, m_iHitDice);
            if (health <= 0) health = 1;
        }
        return;
    }
    ...
    cTargetType = sTarget->get_owner_type();
    if (sTarget != nullptr)
    {
        target = sTarget;
    ...
    cDir = GetNextMoveDir(sX, sY, dX, dY, map_, turn, &m_tmp_iError);
    if (cDir == 0)
    {
    }
    else
            ...
            switch (get_type())
            {
                case 38: // Mana Collector
                    if (m_sBehaviorTurnCount >= 3) {
                        ...
                        if (bFlag == true) {
                            map_->gs->send_event_near_type_a(shared_from_this(), MSGID_MOTION_ATTACK, x, y, 1);
                        }
                    }
                    break;
...
void npc::behavior_attack()
{
    ...
    uint8_t cDir;
    sX = sY = dX = dY = 0;
    uint32_t dwTime = utility::current_time();
    ...
    if ((getDangerValue(dX, dY) > m_cBravery) && (permAttackMode == false) && (m_cActionLimit != 5))
    {
        m_sBehaviorTurnCount = 0;
    ...
    if ((health <= 2) && (utility::gen_rand(1, m_cBravery) <= 3) && (permAttackMode == false) && (m_cActionLimit != 5))
    {
        m_sBehaviorTurnCount = 0;
        m_cBehavior = BEHAVIOR_FLEE;
        return;
    ...
    {
        cDir = utility::cGetNextMoveDir(sX, sY, dX, dY);
        if (cDir == 0) return;
        direction = cDir;
        ...
        if (m_cActionLimit == 5)
        {
            switch (get_type())
            {
                case 89: // AGT
                    if (target)
                    {
                        if (targetType == ot_player)
                        {
                            map_->gs->send_event_near_type_a(shared_from_this(), MSGID_MOTION_ATTACK, dX, dY, 1);
                            m_iMagicHitRatio = 1000;
                            magicHandler(target, dX, dY, 61);
                        }
                    }
                    break;
                case 87: // CT
                    if (target)
                    {
                        if (targetType == ot_player)
                        {
                            map_->gs->send_event_near_type_a(shared_from_this(), MSGID_MOTION_ATTACK, x + _tmp_cTmpDirX[cDir], y + _tmp_cTmpDirY[cDir], 2);
                            map_->gs->CalculateAttackEffect(target, shared_from_this(), dX, dY, 2);
                        }
                    }
                    break;
        ...
        else
        {
            if (m_cMagicLevel == 11)
            {
                map_->gs->send_event_near_type_a(shared_from_this(), MSGID_MOTION_ATTACK, x, y, 1);
                m_iMagicHitRatio = 1000;
                magicHandler(target, x, y, 75);
            }
            else
            {
                map_->gs->send_event_near_type_a(shared_from_this(), MSGID_MOTION_ATTACK, 0, 0, 1);
                map_->gs->CalculateAttackEffect(target, shared_from_this(), dX, dY, 1);
            }
        }
        m_iAttackCount++;
    ...
    else
    {
        ...
        if ((m_cMagicLevel > 0) && (utility::gen_rand(1, 2) == 1) &&
            (abs(sX - dX) <= 9) && (abs(sY - dY) <= 7))
        {
            ...
            {
                case 1:
                    if (map_->gs->m_pMagicConfigList[0]->manaCost <= mana)
                        iMagicType = 0;
                    break;
                case 2:
                    if (map_->gs->m_pMagicConfigList[10]->manaCost <= mana)
                        iMagicType = 10;
                    else if (map_->gs->m_pMagicConfigList[0]->manaCost <= mana)
                        iMagicType = 0;
                    break;
                case 3:
                    if (map_->gs->m_pMagicConfigList[20]->manaCost <= mana)
                        iMagicType = 20;
                    else if (map_->gs->m_pMagicConfigList[10]->manaCost <= mana)
                        iMagicType = 10;
                    break;
                case 4:
                    if (map_->gs->m_pMagicConfigList[30]->manaCost <= mana)
                        iMagicType = 30;
                    else if (map_->gs->m_pMagicConfigList[37]->manaCost <= mana)
                        iMagicType = 37;
                    else if (map_->gs->m_pMagicConfigList[20]->manaCost <= mana)
                        iMagicType = 20;
                    else if (map_->gs->m_pMagicConfigList[10]->manaCost <= mana)
                        iMagicType = 10;
                    break;
                case 5:
                    if (map_->gs->m_pMagicConfigList[43]->manaCost <= mana)
                        iMagicType = 43;
                    else if (map_->gs->m_pMagicConfigList[30]->manaCost <= mana)
                        iMagicType = 30;
                    else if (map_->gs->m_pMagicConfigList[37]->manaCost <= mana)
                        iMagicType = 37;
                    else if (map_->gs->m_pMagicConfigList[20]->manaCost <= mana)
                    ...
                    break;
                case 6:
                    if (map_->gs->m_pMagicConfigList[51]->manaCost <= mana)
                        iMagicType = 51;
                    else if (map_->gs->m_pMagicConfigList[43]->manaCost <= mana)
                        iMagicType = 43;
                    else if (map_->gs->m_pMagicConfigList[30]->manaCost <= mana)
                        iMagicType = 30;
                    else if (map_->gs->m_pMagicConfigList[37]->manaCost <= mana)
                    ...
                    break;
                case 7:
                    if ((map_->gs->m_pMagicConfigList[70]->manaCost <= mana) && (utility::gen_rand(1, 5) == 3))
                        iMagicType = 70;
                    else if (map_->gs->m_pMagicConfigList[61]->manaCost <= mana)
                        iMagicType = 61;
                    else if (map_->gs->m_pMagicConfigList[60]->manaCost <= mana)
                        iMagicType = 60;
                    else if (map_->gs->m_pMagicConfigList[51]->manaCost <= mana)
                        iMagicType = 51;
                    else if (map_->gs->m_pMagicConfigList[43]->manaCost <= mana)
                        iMagicType = 43;
                    break;
                case 8:
                    if ((map_->gs->m_pMagicConfigList[35]->manaCost <= mana) && (utility::gen_rand(1, 3) == 2))
                        iMagicType = 35;
                    else if (map_->gs->m_pMagicConfigList[60]->manaCost <= mana)
                        iMagicType = 60;
                    else if (map_->gs->m_pMagicConfigList[51]->manaCost <= mana)
                ...
                case 9:
                    if ((map_->gs->m_pMagicConfigList[74]->manaCost <= mana) && (utility::gen_rand(1, 3) == 2))
                        iMagicType = 74;
                    break;
                case 10:
                    if ((map_->gs->m_pMagicConfigList[57]->manaCost <= mana) && (utility::gen_rand(1, 3) == 2))
                        iMagicType = 57;
                    break;
                case 11:
                    goto NBA_CHASE;
                    break;
                case 12: // Wyvern
                    if ((map_->gs->m_pMagicConfigList[91]->manaCost <= mana) && (utility::gen_rand(1, 3) == 2))
                        iMagicType = 91;//blizzard
                    else if (map_->gs->m_pMagicConfigList[63]->manaCost <= mana)
                        iMagicType = 63;
                    break;
                case 13: // Abaddon
                    if ((map_->gs->m_pMagicConfigList[96]->manaCost <= mana) && (utility::gen_rand(1, 3) == 2))
                        iMagicType = 96;
                    else if (map_->gs->m_pMagicConfigList[81]->manaCost <= mana)
                        iMagicType = 81;
                    break;
                case 14: // Earth Wyvern
                    if ((map_->gs->m_pMagicConfigList[64]->manaCost <= mana) && (utility::gen_rand(1, 3) == 2))
                        iMagicType = 64;
                    else if (map_->gs->m_pMagicConfigList[96]->manaCost <= mana)
                        iMagicType = 96;
                    break;
                case 15: // Black Demon
                    if (map_->gs->m_pMagicConfigList[92]->manaCost <= mana)
                        iMagicType = 92;
                    else if ((map_->gs->m_pMagicConfigList[73]->manaCost <= mana) && (utility::gen_rand(1, 30) == 2))
                        iMagicType = 73;
                    else if ((map_->gs->m_pMagicConfigList[83]->manaCost <= mana) && (utility::gen_rand(1, 10) == 2))
                        iMagicType = 83;
                    break;
                case 16: // Fire Wyvern
                    if ((map_->gs->m_pMagicConfigList[61]->manaCost <= mana) && (utility::gen_rand(1, 3) == 2))
                        iMagicType = 61;
                    else if (map_->gs->m_pMagicConfigList[81]->manaCost <= mana)
                        iMagicType = 81;
                    break;
                ...
                case 19: // Enraged Cyclops
                    if (map_->gs->m_pMagicConfigList[74]->manaCost <= mana)
                        iMagicType = 74;
                    else if (map_->gs->m_pMagicConfigList[35]->manaCost <= mana)
                        iMagicType = 35;
                    break;
                case 20: // Enraged Gargoyle
                    if ((map_->gs->m_pMagicConfigList[41]->manaCost <= mana) && (utility::gen_rand(1, 3) == 2))
                        iMagicType = 41;
                    else if (map_->gs->m_pMagicConfigList[81]->manaCost <= mana)
                        iMagicType = 81;
                    break;
                case 21: // Enraged Hellclaw
                    if ((map_->gs->m_pMagicConfigList[55]->manaCost <= mana) && (utility::gen_rand(1, 3) == 2))
                        iMagicType = 55;
                    else if (map_->gs->m_pMagicConfigList[35]->manaCost <= mana)
                        iMagicType = 35;
                    break;
                case 22: // Enraged Tigerworm
                    if (map_->gs->m_pMagicConfigList[88]->manaCost <= mana)
                        iMagicType = 88;
                    else if (map_->gs->m_pMagicConfigList[35]->manaCost <= mana)
                        iMagicType = 35;
                    break;
                case 23: // Wind Wyvern
                    if (map_->gs->m_pMagicConfigList[63]->manaCost <= mana)
                        iMagicType = 63;
                    else if (map_->gs->m_pMagicConfigList[93]->manaCost <= mana)
                        iMagicType = 93;
                    break;
            }
            ...
            if (iMagicType != -1)
            {
                if (m_iAILevel >= 2)
                {
                    ...
                    {
                        case ot_player:
                            if (target->magic_effect_status[MAGICTYPE_PROTECT] == MAGICPROTECT_PFM)
                            {
                            ...
                            if ((iMagicType == 35) && (target->magic_effect_status[MAGICTYPE_HOLDOBJECT] != 0)) goto NBA_CHASE;
                            break;
                ...
                map_->gs->send_event_near_type_a(shared_from_this(), MSGID_MOTION_ATTACK, x + _tmp_cTmpDirX[cDir], y + _tmp_cTmpDirY[cDir], 1);
                magicHandler(shared_from_this(), dX, dY, iMagicType);
        ...
        if ((m_cMagicLevel < 0) && (utility::gen_rand(1, 2) == 1) &&
            ...
            if (iMagicType != -1) {
                map_->gs->send_event_near_type_a(shared_from_this(), MSGID_MOTION_ATTACK, x + _tmp_cTmpDirX[cDir], y + _tmp_cTmpDirY[cDir], 1);
                magicHandler(target, dX, dY, iMagicType);
                timeLastAction = dwTime;
                return;
            }
        }
        ...
        if ((attackRange > 1) &&
            ...
            cDir = utility::cGetNextMoveDir(sX, sY, dX, dY);//GetNextMoveDir(sX, sY, dX, dY, pMap, m_cTurn, &m_tmp_iError);//gserver->cGetNextMoveDir(sX, sY, dX, dY);
            if (cDir == 0) return;
            direction = cDir;
            ...
            if (m_cActionLimit == 5)
            {
                switch (get_type()) {
                    ...
                    case 87: // CT
                        if (target && target->is_player())
                        {
                            map_->gs->send_event_near_type_a(shared_from_this(), MSGID_MOTION_ATTACK, dX, dY, 2);
                            map_->gs->CalculateAttackEffect(target, shared_from_this(), dX, dY, 2);
                        }
                        break;
                    case 36: // Crossbow Guard Tower
                ...
                switch (get_type()) {
                    ...
                    case 79:
                        if (target)
                        {
                            if (!target->is_dead() && target->CheckResistIce(direction, m_iMagicHitRatio) == false)
                            {
                                if (target->magic_effect_status[MAGICTYPE_ICE] == 0)
                                {
                                    target->magic_effect_status[MAGICTYPE_ICE] = 1;
                                    target->set_status_flag(STATUS_FROZEN, true);
                                    map_->RegisterDelayEvent(DELAYEVENTTYPE_MAGICRELEASE, MAGICTYPE_ICE, uint64_t(dwTime) + (5 * 1000),
                                        target, 0, 0, 1, 0, 0);
                                }
                            }
                            map_->gs->send_event_near_type_a(shared_from_this(), MSGID_MOTION_ATTACK, dX, dY, 20);
                            map_->gs->CalculateAttackEffect(target, shared_from_this(), dX, dY, 1);
                        }
                        break;
                    ...
                    default:
                        ...
                        //gserver->CalculateAttackEffect(m_iTargetIndex.get(), this, dX, dY, 1);
                        magicHandler(target, dX, dY, 0);//test
                        break;
                }
            }
            m_iAttackCount++;
    ...
    NBA_CHASE:;
        ...
        {
            cDir = utility::cGetNextMoveDir(sX, sY, dX, dY);// GetNextMoveDir(sX, sY, dX, dY, pMap, m_cTurn, &m_tmp_iError);
            if (cDir == 0) {
                return;
            }
            dX = x + _tmp_cTmpDirX[cDir];
...
bool npc::behavior_manaCollector()
{
    ...
    // 							iTotal = utility::gen_rand(1, (g_clientList[sOwnerH]->GetMag()) ); // Mana Point´Â 1D(Magic)
    ...
    switch (m_cMoveType)
    {
        ...
        case MOVETYPE_SEQWAYPOINT:
            ...
            if (m_cCurWaypoint >= m_cTotalWaypoint)
                m_cCurWaypoint = 1;
            m_dX = (short)(map_->waypointList[m_iWayPointIndex[m_cCurWaypoint]].x);
            m_dY = (short)(map_->waypointList[m_iWayPointIndex[m_cCurWaypoint]].y);
            break;
        ...
        case MOVETYPE_RANDOM:
            m_dX = (rand() % (map_->sizeX - 50)) + 15;
            m_dY = (rand() % (map_->sizeY - 50)) + 15;

            //	iMapIndex = m_cMapIndex;
        //
            for (i = 0; i <= 30; i++) {
                sX = (rand() % (map_->sizeX - 50)) + 15;
                sY = (rand() % (map_->sizeY - 50)) + 15;
                ...
                for (j = 0; j < map_->mobGeneratorAvoidList.size(); j++)
                    if (map_->mobGeneratorAvoidList[j].left != -1) {
                        if ((sX >= map_->mobGeneratorAvoidList[j].left) &&
                            (sX <= map_->mobGeneratorAvoidList[j].right) &&
                            (sY >= map_->mobGeneratorAvoidList[j].top) &&
                            (sY <= map_->mobGeneratorAvoidList[j].bottom)) {
                            bFlag = false;
                        }
                    }
...
std::shared_ptr<unit> npc::TargetSearch()
{
    ...
    std::list<std::shared_ptr<unit>> owners = map_->GetOwners(
        x - m_cTargetSearchRange, x + m_cTargetSearchRange,
        y - m_cTargetSearchRange, y + m_cTargetSearchRange);
    if (owners.size() <= 0) return nullptr;
    std::cout << "Owners Size = " << owners.size() << '\n';
    std::cout << "HP = " << this->health << '\n';

    std::shared_ptr<unit> target;

    //TODO: mutex list access
    for (const std::shared_ptr<unit> & owner : owners)
    {
        ...
        switch (owner->get_owner_type())
        {
            case ot_player:
                ...
                iPKCount = map_->gs->GetClient(owner->handle)->playerKillCount;
                break;
            ...
            case ot_npc:
                ...
                if (get_type() == 21) {
                    if (map_->gs->getPlayerNum(map_, owner->x, owner->y, 2) != 0) {
                        continue;
                    }
                }
                break;
    ...
    /* 		else {
            ...
            // INVISIBILITY
        if (owner->is_invisible() && specialAbility != 1) continue;
...
void npc::magicHandler(std::shared_ptr<unit> unit_, short dX, short dY, short magicType)
{
    magic * spell = nullptr;

    if ((dX < 0) || (dX >= map_->sizeX) ||
        (dY < 0) || (dY >= map_->sizeY)) return;
    ...
    if (map_->gs->m_pMagicConfigList[magicType] == nullptr) return;

    if (unit_->is_player())
    {
        if (unit_->map_->flags.attackEnabled == false) return;

        //TODO: Calculate player magic hit ratio (or go ahead as planned and remove it early)
    }
    else
    {
        magicResist = std::static_pointer_cast<npc>(unit_)->m_iMagicHitRatio;
    }

    //TODO: Implement old weather effects?
    // 	iWeatherBonus = gserver->iGetWeatherMagicBonusEffect(spell, pMap->m_weather);

    spell = map_->gs->m_pMagicConfigList[magicType];

    std::shared_ptr<unit> target = map_->GetOwner(dX, dY);
    ...
    if (spell->delayTime == 0) {
        switch (spell->magicType)
        {
            case MAGICTYPE_CANCELLATION:
                if (target) {
                    target->remove_magic_effect(MAGICTYPE_INVISIBILITY);
                    target->remove_magic_effect(MAGICTYPE_PROTECT);
                    target->remove_magic_effect(MAGICTYPE_HOLDOBJECT);
                    target->remove_magic_effect(MAGICTYPE_CONFUSE);
                    target->remove_magic_effect(MAGICTYPE_BERSERK);
                    target->remove_magic_effect(MAGICTYPE_ICE);
                    target->remove_magic_effect(MAGICTYPE_POLYMORPH);
                    target->remove_magic_effect(MAGICTYPE_INHIBITION);
                }
                break;
                    ...
                    case 2: // Detect Invis
                        ...
                        for (short ix = dX - 8; ix <= dX + 8; ix++)
                        {
                            for (short iy = dY - 8; iy <= dY + 8; iy++) {
                                target = map_->GetOwner(ix, iy);
                                if (target) {
                                    target->remove_magic_effect(spell->magicType);
                                    map_->RemoveFromTarget(std::shared_ptr<unit>(target));
                                }
                            }
                        }
                        break;
            ...
            case MAGICTYPE_HOLDOBJECT:
                if (target && target->CheckResistMagic(direction, magicResist) == false) {
                    if (target->is_npc() && (std::static_pointer_cast<npc>(target)->m_cMagicLevel >= 6))
                        break;
                    if (!target->add_magic_effect(spell->magicType, spell->lastTime))
                        break;
                }
                break;
            ...
            case MAGICTYPE_DAMAGE_SPOT:
                ...
                //test
                target = map_->GetOwner(dX, dY);
                if (target && target->is_player()) map_->gs->SendNotifyMsg(0, std::static_pointer_cast<client>(target), NOTIFY_NOTICEMSG, 0, 0, 0, "Magic cast");
                if (target && target->CheckResistMagic(direction, magicResist) == false)
                    map_->gs->Effect_Damage_Spot(unit_, target, spell->m_sValue[MAGICV_THROW], spell->m_sValue[MAGICV_RANGE], spell->m_sValue[MAGICV_BONUS]/* + iWeatherBonus*/, true, spell->element);

                target = map_->GetDeadOwner(dX, dY);
                if (target && target->is_player() && target->health > 0) {
    ...
    map_->gs->SendEventToNearClient_TypeB(MSGID_EVENT_COMMON, COMMONTYPE_MAGIC, map_, x, y, dX, dY, (magicType + 100), get_type());
}
...
{
    int iMaxHP = 0;// = utility::gen_rand(m_iHitDice, 8) + m_iHitDice;
...
void npc::behavior_death(std::shared_ptr<unit> attacker, int16_t dmg)
{
    ...
    if (attacker && attacker->is_player())
    {
        ...
        map_->NpcDeadItemGenerator(attacker, sft());
    }
    ...
    map_->totalAliveObject--;

    map_->RemoveFromTarget(shared_from_this(), ot_npc);
    ...
    map_->gs->send_event_near_type_a(shared_from_this(), MSGID_MOTION_DYING, dmg, sAttackerWeapon, 0);
...
void npc::ReleaseFollowMode(std::shared_ptr<unit> owner)
{
    ...
    for (const std::shared_ptr<npc> & npcs : map_->npclist)
        if (npcs != nullptr) {
            if ((npcs->m_cMoveType == MOVETYPE_FOLLOW) &&
                (npcs->follow == owner) &&
                (npcs->followOwnerType == owner->get_owner_type())) {
